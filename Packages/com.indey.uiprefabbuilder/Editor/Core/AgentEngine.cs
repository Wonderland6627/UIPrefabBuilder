    ...
    public class AgentEngine : IDisposable
    {
        ...
        private void RebuildSystemPrompt()
        {
...

            {
                ...
                sb.AppendLine("- BARS / QUANTITY SLIDERS: use `create_slider` and pass `backgroundSpritePath` + `fillSpritePath` + `handleSpritePath` (plus `value`) in that same call. It builds and wires Background, Fill Area/Fill and Handle Slide Area/Handle. A Slider created without them has NO graphics at all: the mockup's bar disappears while the hierarchy still lists the node, and that is rejected at verdict time.");
                sb.AppendLine("- ICON BUTTONS: a `+` / `-` / arrow button whose sprite already draws the glyph needs NO label — create it with `text=\"\"`. Never write text on top of that artwork.");
                sb.AppendLine("- ICON + LABEL IN ONE BUTTON: use `create_button` with `iconSpritePath` and `iconPosition`, which reserves separate areas for the icon and the label. A loose Image dropped into a button whose label fills the whole face lands on the glyphs (the classic half-hidden price next to a coin) and is rejected at verdict time.");
                sb.AppendLine();
